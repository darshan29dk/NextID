...
import {
  ...
  importRoleAttributes,
  getAttributeCategories
} from '../../services/dashboardService';
import './RoleAttributes.css';
...
const ROLE_TYPES = [
  "IT Role", "Business Role", "Enterprise Role", "Application Role",
  "Functional Role", "Privilege Role", "Composite Role", "Technical Role"
];
...
const RoleAttributes = () => {
  ...
  const fetchAttributes = useCallback(async () => {
    try {
      ...
      console.error(err);
      setErrorMsg(err.response?.data?.detail || 'Failed to fetch role attributes. Please check database connection.');
    } finally {
      setLoading(false);
    }
  }, [page, limit, searchQuery, statusFilter, categoryFilter, roleTypeFilter, dataTypeFilter, requiredFilter, sortBy, sortOrder]);
  ...
  const handleOpenDrawer = async (attr) => {
    try {
      ...
      const response = await getRoleAttribute(attr.id);
  ...
  const handleOpenEditModal = (e, attr) => {
    ...
    setFormData({
      ...
      role_type: attr.role_type || 'IT Role'
    });
    setShowModal(true);
  };
  ...
  const validateForm = () => {
    ...
    if (!formData.attribute_name || !formData.attribute_name.trim()) {
      ...
      errors.attribute_name = "Name must contain only lowercase letters, numbers, and underscores (e.g. role_risk_level)";
    }
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      if (editAttributeId) {
        await updateRoleAttribute(editAttributeId, payload);
      } else {
        await createRoleAttribute(payload);
      }
      setShowModal(false);
      fetchAttributes();
    } catch (err) {
      console.error(err);
      setFormBannerError(err.response?.data?.detail || 'Failed to save role attribute.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDeleteSubmit = async () => {
    ...
    try {
      setSubmitting(true);
      await deleteRoleAttribute(deleteAttr.id);
  ...
  const handleDownloadTemplate = () => {
    const headers = ["Attribute Name", "Display Name", "Data Type", "Role Type", "Description", "Display Order", "Required", "Unique", "Searchable", "Editable", "Status", "Default Value", "Validation Rule"];
    const sampleRow = ["role_sod_flag", "SoD Flag", "Boolean", "IT Role", "Segregation of Duties flag for this role", "22", "FALSE", "FALSE", "TRUE", "TRUE", "Active", "false", ""];
    const csvContent = "data:text/csv;charset=utf-8,"
      + [headers.join(","), sampleRow.join(",")].join("\n");
    const link = document.createElement("a");
    link.setAttribute("href", encodeURI(csvContent));
    link.setAttribute("download", `rAnalyzer_role_attributes_template.csv`);
  ...
  const handleImportSubmit = async (e) => {
    ...
    try {
      setSubmitting(true);
      setImportLogs(null);
      const resp = await importRoleAttributes(importFile);
      setImportLogs(resp);
      fetchAttributes();
    } catch (err) {
      console.error(err);
  ...
  const handleExportCSV = () => {
    const headers = ["Attribute Name", "Display Name", "Role Type", "Data Type", "Category", "Required", "Unique", "Searchable", "Editable", "Status", "Display Order", "Description"];
    const rows = attributes.map(a => [
      a.attribute_name, a.display_name, a.role_type || "IT Role", a.data_type,
      a.category?.category_name || "General",
      a.is_required ? "YES" : "NO", a.is_unique ? "YES" : "NO",
      a.is_searchable ? "YES" : "NO", a.is_editable ? "YES" : "NO",
      a.status, a.display_order, a.description || ""
    ]);
    const csvContent = "data:text/csv;charset=utf-8,"
      + [headers.join(","), ...rows.map(r => r.map(val => `"${String(val).replace(/"/g, '""')}"`).join(","))].join("\n");
    const link = document.createElement("a");
    link.setAttribute("href", encodeURI(csvContent));
    link.setAttribute("download", `rAnalyzer_role_attributes_${new Date().toISOString().slice(0, 10)}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    setShowExportDropdown(false);
  ...
  const handleExportExcel = () => {
    ...
    link.setAttribute("download", `rAnalyzer_role_attributes_${new Date().toISOString().slice(0, 10)}.xls`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    setShowExportDropdown(false);
  ...
  return (
    <div className="role-attributes-page">
      <Breadcrumb items={[
        { label: 'Data Foundation', active: false },
        { label: 'Role Attributes', active: true }
      ]} />
      ...
      <div className="page-header-actions">
        <div className="header-title-section">
          ...
          <p>Define metadata structure and schema for roles used in mining, certification, and access governance.</p>
        </div>
      ...
      <div className="attributes-kpi-grid">
        <DashboardCard title="Total Attributes" value={kpiStats.total} icon={Layers} color="blue" loading={loading} onClick={() => handleKpiClick('total')} />
      ...
      <div className="attributes-toolbar-wrapper">
        ...
        <div className="filters-dropdowns-row">
          ...
          <select value={roleTypeFilter} onChange={e => { setRoleTypeFilter(e.target.value); setPage(1); }}>
      ...
      <div className="attributes-table-container">
        <div className="attributes-table-header role-columns">
        ...
        {loading ? (
          <div className="table-loading-container">
            ...
            <p className="text-muted" style={{ fontSize: '13px' }}>Loading role attributes schema...</p>
          </div>
        ) : attributes.length === 0 ? (
          <div className="table-empty-container">
            ...
            <h4>No Role Attributes Found</h4>
            <p>Click 'Add Attribute' or 'Import' to establish role schema.</p>
          </div>
        ) : (
          <div className="attributes-table-body">
            {attributes.map((attr, idx) => (
              <div
                key={attr.id}
                className="attributes-table-row role-columns"
                onClick={() => handleOpenDrawer(attr)}
                title="Click to view details and usage"
              >
                ...
                {/* Role Type */}
                <div>
                  <span className="attr-role-badge">{attr.role_type || "IT Role"}</span>
                </div>
        ...
        {!loading && total > 0 && (
          <div className="table-pagination-bar">
            <span className="pagination-count">
              Showing <b>{total === 0 ? 0 : (page - 1) * limit + 1}</b> to <b>{Math.min(page * limit, total)}</b> of <b>{total}</b> role attributes
            </span>
        ...
        <div className="modal-overlay-custom">
          <div className="modal-content-custom">
            <div className="modal-header-custom">
              <h3>{editAttributeId ? 'Edit Role Schema' : 'Add Role Attribute'}</h3>
              ...
              <div className="modal-body-custom">
                ...
                <div className="form-grid-2col">
                  ...
                  <div className="form-group-custom">
                    ...
                    <input
                      ...
                      placeholder="e.g. role_risk_level"
                  ...
                  <div className="form-group-custom">
                    ...
                    <input
                      ...
                      onChange={handleInputChange}
                      placeholder="e.g. Role Risk Level"
                      disabled={editAttributeId && (formData.is_system || formData.attribute_type === 'System')}
                ...
                <div className="modal-flags-checkboxes-row">
                  <label className="checkbox-custom-label">
                    <input type="checkbox" name="is_required" checked={formData.is_required}
                      disabled={editAttributeId && (formData.is_system || formData.attribute_type === 'System')}
                      onChange={handleInputChange} />
                    <span>Required</span>
                  </label>
                  <label className="checkbox-custom-label">
                    <input type="checkbox" name="is_unique" checked={formData.is_unique}
                      disabled={editAttributeId && (formData.is_system || formData.attribute_type === 'System')}
                      onChange={handleInputChange} />
                    <span>Unique</span>
                  </label>
                  <label className="checkbox-custom-label">
                    <input type="checkbox" name="is_searchable" checked={formData.is_searchable}
                      disabled={editAttributeId && (formData.is_system || formData.attribute_type === 'System')}
                      onChange={handleInputChange} />
                    <span>Searchable</span>
                  </label>
                  <label className="checkbox-custom-label">
                    <input type="checkbox" name="is_editable" checked={formData.is_editable}
                      disabled={editAttributeId && (formData.is_system || formData.attribute_type === 'System')}
                      onChange={handleInputChange} />
                    <span>Editable</span>
                  </label>
            ...
            <form onSubmit={handleImportSubmit}>
              <div className="modal-body-custom">
                <p style={{ fontSize: '13px', color: 'var(--text-muted)' }}>
                  Ingest role schema definitions from a formatted CSV. Download the template structure for reference.
                </p>
                ...
                <div className="file-drop-area">
                  ...
                  />
                  <label htmlFor="import-csv-uploader-role" className="drop-lbl">
                    <Upload size={20} />
                ...
                {drawerActiveTab === 'overview' && (
                  <div className="drawer-tab-content animate-fade">
                    <div className="drawer-section">
                      ...
                      <p className="drawer-desc-txt">{drawerAttr.description || "No description provided for this role attribute."}</p>
                    ...
                    <div className="drawer-section">
                      ...
                      <div className="properties-list-grid">
                        <div className="prop-row">
                          <span className="prop-lbl">Role Type</span>
                          <span className="prop-val text-highlight">{drawerAttr.role_type || "IT Role"}</span>
                        </div>
                        <div className="prop-row">
                      ...
                      {drawerUsage ? (
                        <div className="usage-stats-grid">
                          <div className="usage-card">
                            <span className="usage-num">{drawerUsage.platform_roles_count}</span>
                            <span className="usage-lbl">Platform Roles</span>
                          </div>
                          <div className="usage-card">
                            <span className="usage-num">{drawerUsage.role_mining_count}</span>
                            <span className="usage-lbl">Role Mining</span>
                          </div>
                          <div className="usage-card">
                            <span className="usage-num">{drawerUsage.role_catalog_count}</span>
                            <span className="usage-lbl">Role Catalog</span>
                          </div>
                          <div className="usage-card">
                            <span className="usage-num">{drawerUsage.birthright_roles_count}</span>
                            <span className="usage-lbl">Birthright Roles</span>
                          </div>
                          <div className="usage-card">
                            <span className="usage-num">{drawerUsage.access_requests_count}</span>
                            <span className="usage-lbl">Access Requests</span>
                          </div>
                          <div className="usage-card">
                            <span className="usage-num">{drawerUsage.access_certification_count}</span>
                            <span className="usage-lbl">Certifications</span>
                          </div>
                        </div>
                      ) : (
                    ...
                    <div className="drawer-section" style={{ marginTop: '16px' }}>
                      ...
                      <div className="info-notes-block">
                        ...
                        <p style={{ fontSize: '12px', margin: 0 }}>
                          Changes to role attribute definitions propagate across all linked role schemas, mining jobs, and active certification campaigns.
                        </p>
                      </div>
                    </div>
                  </div>
